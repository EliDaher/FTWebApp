...
export default function AddWorkOut() {
  ...
  useEffect(() => {
    ...
    if (workoutData) {
      ...
      setLevel(workoutData.level || 'Beginner')
      setDuration(workoutData.duration || 30)
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (workoutData) {
        // تعديل موجود باستخدام PUT
        const response = await axios.put(`https://ftserver-ym6z.onrender.com/workout/${workoutData.id}`, {
          updatedWorkOut: workoutPayload
        })
        console.log('✅ Updated:', response.data)
        alert('تم تعديل البرنامج بنجاح')
      } else {
        // إنشاء جديد باستخدام POST
        const response = await axios.post('https://ftserver-ym6z.onrender.com/addWorkOut', {
          newWorkOut: workoutPayload
        })
